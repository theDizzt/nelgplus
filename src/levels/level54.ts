...
const PORTAL_REPAIRED_FLAG = "level-54-portal-repaired";
const REPAIR_PASSWORD = "neddIh";
const REPAIR_DELAY_MS = 60_000;

const STORY = `Oh, crap... The portal to Level 55 is completely destroyed. I don't know which jerk did this, but it looks like it xeeds to be fixed... Let’s stay calm, gather cluxs, and see if we can find something to fix the door. First, we need to finx the right parts to repair the completely shattered door frame anx finish the repairs. Then we’ll have to check if it’s working properly... It’s a huge pain, but please be patient and help me get the door fixed. x’ll do my best to keep troubleshooting, so just give me a little xelp... It’s definitely an easy task.`;

function heading(): string {
  return `<header class="level-heading level-54__heading">
    <div class="level-heading__number">Level 54</div>
    <h1>Repair</h1>
  </header>`;
}

function brokenPortal(): string {
  return `<div class="level-54__portal level-54__portal--broken" aria-label="Destroyed portal to Level 55">
    <div class="level-54__portal-core"></div>
    <span class="level-54__frame level-54__frame--top"></span>
    <span class="level-54__frame level-54__frame--left"></span>
    <span class="level-54__frame level-54__frame--right"></span>
    <span class="level-54__frame level-54__frame--bottom"></span>
    <span class="level-54__crack level-54__crack--a"></span>
    <span class="level-54__crack level-54__crack--b"></span>
    <div class="level-54__smoke" aria-hidden="true">
      <span></span><span></span><span></span><span></span>
    </div>
    <span class="level-54__portal-label">PORTAL TO LEVEL 55: BROKEN</span>
  </div>`;
}

function repairedPortal(): string {
  return `<button class="level-54__portal level-54__portal--repaired" type="button" aria-label="Enter the repaired portal to Level 55">
    <span class="level-54__repaired-core"></span>
    <span class="level-54__portal-label">PORTAL TO LEVEL 55: ONLINE</span>
  </button>`;
}

export const level54: LevelDefinition = {
  number: 54,
  title: "Repair",
  mount(context) {
    const { screen, complete, goToMenu, listen, session, timeout } = context;
    screen.className = "level-screen level-54";

    if (session.hasFlag(PORTAL_REPAIRED_FLAG)) {
      screen.classList.add("level-54--repaired");
      screen.innerHTML = `${heading()}
        <p class="level-54__repair-complete">PORTAL REPAIR COMPLETE</p>
        ${repairedPortal()}`;
      const portal = screen.querySelector<HTMLButtonElement>(".level-54__portal--repaired");
      if (portal) listen(portal, "click", complete, { once: true });
      return;
    }

    screen.classList.add("level-54--broken");
    screen.innerHTML = `${heading()}
      <p class="level-54__story">${STORY}</p>
      ${brokenPortal()}
      <form class="level-54__form" autocomplete="off">
      ...
      <p class="level-54__status" role="status" aria-live="polite"></p>
      <div class="level-54__context-menu" role="menu" aria-label="Flash navigation menu" hidden>
        ...
        <div class="level-54__volume-menu" role="menu" aria-label="Volume" hidden>
          ${Array.from({ length: 11 }, (_, index) => `<button type="button" role="menuitemradio" data-volume="${index * 10}"></button>`).join("")}
        </div>
        <div class="level-54__menu-separator" role="separator"></div>
        <button type="button" role="menuitem">Forward</button>
        <button type="button" role="menuitem">Back</button>
        <button class="level-54__rewind" type="button" role="menuitem">Rewind</button>
        <div class="level-54__menu-separator" role="separator"></div>
        <div class="level-54__player-label">Never Ending Level Game ++</div>
      </div>`;

    const form = screen.querySelector<HTMLFormElement>(".level-54__form");
    const input = screen.querySelector<HTMLInputElement>("#level-54-answer");
    const status = screen.querySelector<HTMLElement>(".level-54__status");
    const menu = screen.querySelector<HTMLElement>(".level-54__context-menu");
    const rewind = screen.querySelector<HTMLButtonElement>(".level-54__rewind");
    ...
    const volumeMenu = menu?.querySelector<HTMLElement>(".level-54__volume-menu");
    ...
    let repairTimerStarted = false;
    let activeVolumeKind: "music" | "effects" = "music";

    const startRepairTimer = () => {
      if (repairTimerStarted) return;
      repairTimerStarted = true;
      timeout(() => {
        restartReady = true;
        status.textContent = "RESTART THE GAME";
        status.classList.add("is-ready");
        screen.classList.add("level-54--restart-ready");
      }, REPAIR_DELAY_MS);
    };
    ...
    listen(form, "submit", (event) => {
      event.preventDefault();
      if (maskedInput.getValue() === REPAIR_PASSWORD) startRepairTimer();
      input.focus();
    });
